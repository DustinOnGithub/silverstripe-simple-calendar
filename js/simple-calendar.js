jQuery('document').ready(function() {
	...
	window.simplecalendarOptions = {
		...
		eventSources: [
			{
				url: window.location.pathname.replace(/\/\s*$/, "") + '/entriesasjson',
				type: 'POST',
				error: function() {
...
ec.on('reloadListViewEntries', function(payload) {
	$.ajax({
		url: window.location.pathname.replace(/\/\s*$/, "") + '/reloadlistviewentries',
...
ec.on('reloadCalendarLegend', function(payload) {
	$.ajax({
		url: window.location.pathname.replace(/\/\s*$/, "") + '/reloadcalendarlegend',
		context: document.body,
		type: 'POST',
		data: {
			start: $('.calendar--calendar-view').fullCalendar('getView').start.format(),
